...
- 初版后对话改片：对话 AI 读取当前工程代码和验收记录，判断用户需要完整重新生成、单幕修复还是局部微调，并由用户点击按钮后执行。
...
React 网页工作台
  ...
  -> 初版后：对话 AI 读取最新工程，给出重新生成、单幕修复或微调按钮
       -> 重新生成：整理用户需求给提示词 AI，完整生成新工程
       -> 单幕修复：只重画指定幕，其他幕保持不变，再重新本地渲染完整视频
...
10. 只需重画某一幕时显示“单幕修复”按钮，点击后只重新生成指定幕，其余幕不变，再重新本地渲染完整视频。
11. 只需局部修改时显示“微调”按钮，点击后由对话 AI 修改现有 HyperFrames 工程文件，再重新本地渲染和验收。
...
- 初版后对话改片由对话 AI 分流：完整重新生成交给提示词 AI，单幕修复只重画指定幕，局部微调由对话 AI 修改现有工程文件。
